...
class Core:
    ...
    class Chrome:
        ...
        def RunDriver(path: str | None = None, json_string: str | None = None) -> None:
            ...
            for uname, unit in units.items():
                try:
                    ...
                    for aname, action in actions.items():
                        ...
                        log_line = f"[Unit:{uname}][Action:{aname}] of type \'{action['type']}\' successfully executed"
                        Support.log_proc(parent_log_path, log_line)
            ...
            path = "log.txt"
                        #f"{parent_log_path}/ltlogs/{today}.log" átmenetileg van kikommentelve
    ...
    def logJS(
        ...
        if terminal_mode:
            if isinstance(log, SeJSException):
                ...
                raise ValueError("logJS takes either a string or a selenium.common.exceptions.JavascriptException as an argument")
