...
class Support:
    ...
    def log_all(path_: str, log_line: str, time_disabled: bool = False) -> None:
        ...
        try:
            ...
            with open(path_t, mode='w', encoding='utf-8') as f:
                f.write("")

        with open(path_t, "a", encoding="utf-8") as f:
    ...
    def logJS(
        ...
        if terminal_mode:
            if isinstance(log, SeJSException):
                ...
                with open(path_t, mode ='a+', encoding='utf-8') as f:
                    if index is not None:
                        to_write = f"{index} ❗ JavaScript:\\n{log}\n"
                    else:
                        to_write = f"❗ JavaScript:\\n{log}\n"
            ...
            while True:
                with open(path_t, mode = 'r', encoding = "utf-8") as f:
                    contents: str = f.read()
                if contents != '':
                    sleep(fuckup)
                    continue
    ...
    def auto_logJS(
        ...
        try:
            ...
            with open(path_t, mode='w', enocdintóg='utf-8') as f:
                f.write("")
